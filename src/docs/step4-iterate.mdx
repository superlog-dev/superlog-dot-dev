...
If that is the case, maintaining the simple 1 commit per pull request modal, and using amend to update that 1 commit will have no differences in the result.
But the workflow suddenly becomes much simpler, and merge conflicts are easier to resolve.

The downside is that GitHub support for comparing each force-push is not very good.
But this is somewhat bearable because people tend to create smaller, easier-to-review pull requests using Superlog.
